...
const regex = /(export default Home;)/;

const newCode = `function BudgetCategoryDetailsModal({
  category,
  onClose,
  transactions,
  language
...
let newContent2 = content.replace(regex, newCode);
...
newContent2 = newContent2.replace(regex3, newCode3);

fs.writeFileSync('src/components/Home.tsx', newContent2);
